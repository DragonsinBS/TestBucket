...
def create_test(request,course_id):
    if request.method=='GET':
        if request.user.is_authenticated:
            return render(request,'webapp/create_test.html',{"course_id":course_id})
    ...
    try:
        ...
        for ct in range(0,count):
            ...
            if(responses[ct]!=correct[ct]):
                score=0
                question=Question.objects.get(question_id=ct+1)
...
def responses_view(request,test_id):
    ...
    return render(request,"webapp/responses.html",{"questions":res_list,"total_score":total.total_marks,"test_id":test.test_id,"total":responses.count()})
...
def test_summary(request,test_id):
    ...
    return render(request,"webapp/test_summary.html",{"test_id":test.test_id,"students":students})
...
def student_dashboard(request):
    ...
    for course in opted_courses:
        print(course.course_id.course_id)
...
def student_course(request,course_id):
    student=Student.objects.get(student_name=request.session['user_name'])
    course=Course.objects.get(course_id=course_id)
    completed_tests=list(TotalScore.objects.filter(student_id=student,course_id=course).values('test_id','total_marks'))
    com_values=[]
    for item in completed_tests:
        com_values.append(item['test_id'])
